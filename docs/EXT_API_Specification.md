...
"주의: 본 문서에 정의되지 않은 필드는 반드시 동봉된 Kiwoom REST API 문서.pdf의 해당 TR 섹션을 기준으로 구현한다. Naver API는 공식 개발자 센터 문서를 참조한다."
**모든 필드는 키움증권 Open API 명세서(PDF)를 기준으로 기술되었습니다. 만약 존재하지 않는 필드가 있다면 `키움 REST API 문서`를 확인합니다.**
